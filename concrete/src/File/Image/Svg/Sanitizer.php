...
class Sanitizer
{
    ...
    public function sanitizeData($data, ?SanitizerOptions $options = null, array &$removedNodes = [])
    {
        ...
        if (!is_string($enshrinedData)) {
            // The library returns false when it can't parse its input: returning that as-is
            // would make sanitizeFile() write an empty file over the original one.
            throw SanitizerException::create(SanitizerException::ERROR_FAILED_TO_PARSE_XML);
        }
        // The enshrined/svg-sanitize library may have detected and removed additional unsafe
        // content that our own allowlist/blocklist checks above didn't catch (eg javascript:
        // URIs). Record it so that reject-mode callers (checkData()) don't silently accept a
        // file that sanitize-mode would have cleaned up.
        // Note that we can't detect that by comparing the data we gave the library with the
        // data it gave back: it always reformats its output (it indents it, it adds the XML
        // declaration, it expands self-closing tags, it reorders attributes), so the two would
        // differ for every single SVG - including perfectly safe ones. Let's ask the library
        // what it actually removed instead.
        $enshrinedRemovedNodes = $this->countEnshrinedRemovedNodes();
        if ($enshrinedRemovedNodes > 0) {
            if (isset($removedNodes['enshrined'])) {
                $removedNodes['enshrined'] += $enshrinedRemovedNodes;
            } else {
                $removedNodes['enshrined'] = $enshrinedRemovedNodes;
            }
        }
    ...
    /**
     * Count the nodes that the enshrined/svg-sanitize library removed in its last run.
     *
     * @return int
     */
    protected function countEnshrinedRemovedNodes()
    {
        $result = 0;
        foreach ($this->enshrinedSvgSanitizer->getXmlIssues() as $issue) {
            // The library also strips comments, which are harmless: reporting them would
            // mean rejecting most of the SVG files generated by the usual drawing tools.
            if (isset($issue['message']) && strpos($issue['message'], "'#comment'") !== false) {
                continue;
            }
            ++$result;
        }
